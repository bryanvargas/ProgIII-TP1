...
public class GeneradorFrases implements Iterable<Frase> {
...
//		generado = new TreeMap<Integer, ArrayList<Frase>>();
...
//			 addFrase(new Frase(a.get(0),a.get(1),a.get(2),a.get(3)));		
//		}	
//		Iterator<Integer> it = generado.keySet().iterator();
//		while(it.hasNext()){
//			Integer key = it.next();
//			addFrase(new Frase(a.get(key).get(0),a.get(key).get(1),a.get(key).get(2),a.get(key).get(3)));
//		addFrase(generador());
//		}
...
//			System.out.println(palabra(1));
	...
	public void generador2(){			
		for (int j = 0; j < getCantidad(); j++) {				 			  
			...
			miFrase.add(palabra(0));
			miFrase.add(palabra(1));
//			System.out.println(miFrase.add(palabra(2)));
